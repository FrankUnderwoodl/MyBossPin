...
import com.yufeng.properties.ExcludeUrlProperties;
...
public class JWTSecurityFilter extends BaseInfoProperties implements GlobalFilter, Ordered {
    ...
    /**
     * 过滤器的核心逻辑
     * @param exchange 包含请求和响应信息的容器, 你可以类比成HttpServletRequest和HttpServletResponse的组合体，也就是说：①在转发请求给微服务时，可以进行一个链路的处理 ②微服务们返回数据给前端的时候，也能进行一个链路的上处理
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 2.获取不需要token验证的地址列表(白名单)、静态资源的访问路径
        List<String> excludeList = excludeUrlProperties.getUrls();
        String fileRelease = excludeUrlProperties.getFileRelease();

        if (StringUtils.isNotBlank(fileRelease)) {

            // matchStart：判断当前请求路径是否在静态资源的访问路径中,举个例子：
            // 如果fileRelease是"/static/**"，而getUrl是"/static/css/style.css"，那么matchStart会返回true
            boolean matchStart = antPathMatcher.matchStart(fileRelease, getUrl);
            if (matchStart) {
                log.info("当前请求路径 '{}' 在静态资源的访问路径中，直接放行请求！", getUrl);
                return chain.filter(exchange); // 如果在静态资源的访问路径中，直接放行请求到下一个Handler
            }
        }
        ...
        if(excludeList != null && !excludeList.isEmpty()) {
            ...
            if (excludeList.contains(getUrl)) {
                ...
                return chain.filter(exchange); // 如果在列表中，直接放行请求到下一个Handler
            }
        ...
        // 到达此处表示被拦截了，需要进行JWT验证
        // log.info("当前请求路径 {} 不在不需要验证的地址列表中，下面将进行JWT验证啦！", getUrl);


        // 4.如果请求路径不在不需要验证的地址列表中，下面就进行JWT验证逻辑
        // 例如：从请求头中获取JWT，解析并验证它的有效性
        // 这里前后端规约了一个规则：默认都是从header中获取用户的token
        HttpHeaders headers = exchange.getRequest().getHeaders();
        // getFirst：获取指定名称的header的第一个值(防御性编程：在HTTP协议中，确实允许请求头中存在多个相同名称的header字段，每个字段可以有不同的值)
        ...
        log.info("前端有新的请求到来，IP地址为：{}, 端口为：{}, 接口路径为：{}，header中的令牌为：{}",
                clientIp, clientPort, getUrl, userToken);

        // 判空header中的令牌
        if (StringUtils.isNotBlank(userToken)) {

            String[] tokenArr = userToken.split(JWTUtil.AT);
            // 验证tokenArr的长度是否为2，也就是查看前端传过来的JWT格式正确，因为JWT的格式是：前缀.令牌
    ...
    public Mono<Void> dealJWT(String jwt, ServerWebExchange exchange, GatewayFilterChain chain, String key) {
        ...
        try {
            ...
            // 如果有效之后，将用户信息放入请求头中，让网关转发请求的时候可以携带用户信息
            exchange = setNewHeader(exchange, key, userJson); // 设置新的header(key,value)，将用户信息放入请求头中

            // 如果不抛出异常，则交给下一个过滤器处理
            // log.info("JWT验证通过啦，用户信息为: {}", userJson);
            ...
            // 如果验证失败，抛出自定义异常
            log.error("JWT验证失败，错误的信息为: {}", e.getMessage());
            e.printStackTrace();
    ...
    /**
     * @describe 作用：设置新的header，将用户信息放入Request头中，格式是：key为端的前缀，value为用户信息的JSON字符串
     * @param exchange 包含请求和响应信息的容器, 类似于HttpServletRequest和HttpServletResponse的组合体
    ...
    public ServerWebExchange setNewHeader(ServerWebExchange exchange, String key, String userJson) {
       ...
       /*  // 这里的exchange是一个容器，包含了请求和响应的信息
        ...
        ServerHttpRequest mutatedRequest = exchange.getRequest()
                .mutate() // 从原来的请求中，创建一个新的请求对象
                .header(key, userJson)  // 添加新的header，将用户信息放入其中
                .build();
    ...
    /**
     * 作用： 返回执行优先级，0表示最高优先级，此处这里是jwt校验嘛，所以优先级最高
