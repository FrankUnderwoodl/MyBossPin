 ...
 * @Description RabbitMQ延迟队列的配置类(专门针对简历刷新次数上限的延迟队列)
 *
 */
...
public class DelayConfig_MaxCounts {
    ...
    public Exchange exchange() {
        return ExchangeBuilder
                .topicExchange(EXCHANGE_DELAY_MAX_COUNTS) // 声明一个主题交换机
    ...
    public Queue queue() {
        ...
        return QueueBuilder
                .durable(QUEUE_DELAY_MAX_COUNTS)  // 声明一个持久化队列，也就是重启RabbitMQ服务器后仍然存在
                .build();
    }
    ...
    public Binding delayBindingMaxCounts(@Qualifier(EXCHANGE_DELAY_MAX_COUNTS) Exchange exchange,
        ...
        return BindingBuilder
                .bind(queue)
                .to(exchange)
                .with(DELAY_MAX_COUNTS_REFRESH) // 路由键，#表示匹配任意字符，比如 delay.refresh.industry；*表示匹配一个字符，比如 delay.refresh.industry
                .noargs(); // 不需要额外参数
    }
